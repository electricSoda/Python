...
#images
os.chdir('C:\Computer Science\HelloWorld2_source_code\Skier')
...
def mainloop():
    global running, speed, map_position, points, score_text
    running = True
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:  # check for key presses
                if event.key == pygame.K_LEFT:  # left arrow turns left
                    ...
                    speed = skier.turn(1)
                elif event.key == pygame.K_ESCAPE:  # escape pauses the program
                    if running == True:
                        running = False
                    pause()
